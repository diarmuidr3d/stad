...
class RealTimeItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (stop.operator == Operator.IarnrodEireann ||
        stop.operator == Operator.Luas) {
    ...
    if (timing.realTime) {
      return Row(
        ...
        children: <Widget>[
          ...
          Text(
            "${timing.dueMins} ${Strings.shortenedMinutes}",
...
class RealTimePageState extends State<RealTimePage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (isFavourite == null)
      Favourites()
          .isFavourite(stopData.stop.stopCode)
          .then((isFav) => setState(() => isFavourite = isFav));
    return Scaffold(
      appBar: AppBar(
        ...
        actions: <Widget>[
          IconButton(
              ...
              onPressed: () => {displayStopReal(stopData.stop)}),
          IconButton(
              icon: isFavourite != null && isFavourite
                  ? Icon(
                      Icons.favorite,
                      color: Colors.red,
                    )
